...
for historical issues. The testing philosophy draws on the software
certification principles of \citet{gould2001certification}; the full
framework is documented in Appendix~\ref{app:qa}.
...
or API endpoint deprecations. The test suite comprises 89 integrated tests
distributed across 17 categories, covering environment validation, data download
...
Table~\ref{tab:test-coverage} summarizes the composition.

\begin{table}[H]
\centering
\caption{Test suite composition (89 tests across 17 categories)}
...
FMT   & Output format     & 7  & Wide, long, latest, nobasic, describe, date \\
CTRY  & Country options   & 11 & Basic, full, geo, ISO, regions, income, admin \\
...
CACHE & Cache management  & 8  & Init, path resolution, clear, version, timestamps \\
SYNC  & Sync system       & 5  & Check, download, force, up-to-date, discovery \\
DISC  & Discovery         & 7  & Sources, topics, search, filter, info \\
...
Local metadata caches are managed through the \texttt{sync} family of
commands documented in Section~\ref{sec:sync}.  The design separates
\emph{preview} from \emph{apply}: the default \texttt{sync} is a dry run
that shows what would change, while \texttt{sync replace} downloads and
installs the latest YAML definitions and country classifications from
GitHub.  Three auxiliary commands---\texttt{checkupdate},
\texttt{cacheinfo}, and \texttt{clearcache}---provide version queries and
cache housekeeping.  The legacy \texttt{update} sub-commands
(\texttt{update query}, \texttt{update check}, \texttt{update all}) remain
functional but are deprecated in favor of this unified interface.
...
accessible through the World Bank API---over 29{,}000 records spanning 51 data
...
The \texttt{sync} command provides version-controlled metadata updates with an
explicit safety gate. The default behavior is a \emph{dry run} that displays a
preview of what would change without modifying the local cache. Applying changes
requires the \texttt{replace} modifier, ensuring that metadata updates are never
accidental.

\hangpara
\texttt{sync} previews metadata changes without applying them. This is the safe
default: it reports the local cache version, compares it against the latest
available release, and displays summary statistics.

\hangpara
\texttt{sync detail} extends the preview with per-source and per-topic indicator
breakdowns, showing how many indicators each source contributes and how they are
distributed across topic categories.

\hangpara
\texttt{sync replace} applies the metadata synchronization. After displaying the
same preview, it downloads the latest metadata release from the GitHub repository
and updates the local cache files.

\hangpara
\texttt{sync force} forces a fresh preview by re-querying the API rather than
relying on cached diagnostics. This remains a dry run; no changes are applied.

\hangpara
\texttt{sync replace force} combines both modifiers: it forces a fresh download
of metadata regardless of the local cache version, bypassing staleness checks.

\noindent Three additional cache management commands complement the sync system:

\hangpara
\texttt{checkupdate} queries the remote repository for the latest release version
and reports whether an update is available, without downloading or modifying any
files.
...
\hangpara
\texttt{clearcache} removes the local metadata cache entirely, forcing a full
re-download on the next sync or discovery command.

\noindent For backward compatibility, several deprecated options continue to
function with a warning.  The sync aliases \texttt{syncforce},
\texttt{syncpreview}, and \texttt{syncdryrun} map transparently to
\texttt{sync replace force}, \texttt{sync replace}, and \texttt{sync},
respectively.

The \texttt{metadataoffline} option, which previously generated
71 per-indicator \texttt{.sthlp} files ($\sim$15\,MB), is superseded by the
YAML metadata architecture and the discovery commands described in
...
an aspirational norm to a default behavior. Version~18.0 extends this approach
...
publication-ready metadata handling, and an 89-test QA suite across 17 categories.
...
\subsection{Sync: Metadata status preview}\label{app:diagnostics:sync}
\begin{stlog}
\input{sjlogs/ex_sync_preview.log.tex}\nullskip
\end{stlog}

\subsection{Sync: Detailed metadata breakdown}\label{app:diagnostics:syncdetail}
\begin{stlog}
...
this approach, version~18.2 introduces \emph{deterministic offline tests}
...
total suite to 89~tests across 17~categories.
