...
Layer~2 adds 71~tests organized in 12~categories, from
discovery commands through data cache workflows (Appendix~\ref{app:qa}).
Table~\ref{tab:test-coverage} summarizes the Layer~1 composition.
...
\hline
\multicolumn{4}{@{}l}{\textit{Layer~1: Feature validation (\texttt{qa/run\_tests.do})}} \\
ENV        & Environment       & 5  & Network, API, YAML file presence \\
DL         & Download modes    & 5  & Indicator, country, multi-indicator \\
FMT        & Output format     & 3  & Long format, year range, latest \\
CTRY       & Country options   & 10 & Match, full, geo, ISO, regions, income \\
REG        & Regression        & 4  & Historical bug fixes (\#33, \#45, \#46, \#51) \\
LW         & Linewrap          & 4  & Metadata wrapping, formats, latest scalars \\
UPD        & Maintenance       & 6  & Legacy update, describe, sync interop \\
TOPIC/LANG & Topics \& language & 2  & Topic download, Spanish metadata \\
Advanced   & Advanced features & 6  & Projection, nobasic, describe, nometadata, date \\
CACHE/SYNC & Cache \& sync     & 13 & Cache lifecycle, sync operations, cross-validation \\
DISC       & Discovery         & 10 & Sources, topics, search, filter, info, multi-topic \\
CHAR       & Characteristics   & 6  & Dataset/variable \texttt{char}, persistence, \texttt{nochar} \\
ERR        & Error conditions  & 8  & Syntax validation (\texttt{rcof}), API errors, empty \\
EXT        & Extreme cases     & 4  & Minimal queries, large datasets, edge inputs \\
DET        & Deterministic     & 6  & Offline fixtures, value pinning, network-free \\
           & \textit{Subtotal} & \textit{92} & \\
\hline
\multicolumn{4}{@{}l}{\textit{Layer~2: Documentation validation (\texttt{tests/test\_help\_examples.do})}} \\
HELP-01..24  & Discovery commands     & 25 & Sources, topics, search, info, browse \\
HELP-25..26  & Deprecated options     & 2  & \texttt{update query}, \texttt{update check} \\
HELP-27..29c & Cache management       & 5  & Sync, cache info, clear data cache \\
HELP-29..41a & Data downloads         & 13 & Indicator, country, topic, multi, projection \\
HELP-41..44  & Linewrap               & 4  & Metadata wrapping, multi-indicator, latest \\
HELP-45      & Match                  & 1  & \texttt{match(countrycode)} merge \\
HELP-46..53  & Named examples         & 8  & Click-to-run \texttt{wbopendata\_examples} \\
HELP-54      & Discovery workflow     & 1  & End-to-end: sources $\to$ search $\to$ download \\
EX-01..08    & Inline example replay  & 8  & Data assertions, row counts, graph output \\
EX-09        & Stored results         & 1  & \texttt{r()} values after \texttt{linewrap latest} \\
EX-10..11    & Characteristic metadata & 2  & \texttt{char \_dta[]}, \texttt{nochar} suppression \\
EX-12        & Data cache workflow    & 1  & Cache lifecycle: API $\to$ hit $\to$ reset $\to$ re-fetch \\
             & \textit{Subtotal}      & \textit{71} & \\
\hline
