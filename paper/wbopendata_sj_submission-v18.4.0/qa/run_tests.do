...
local logs_dir "`qadir'/logs"
cap mkdir "`logs_dir'"
local logfile "`logs_dir'/test_results_v`version'_`datestr'.log"
local histfile "`logs_dir'/test_history.txt"
...
if $skip_test == 0 {
    cap noi {
        * Search WDI source (source 2) — most indicators have topics
        qui _wbopendata_search GDP, source(2) limit(10)
...
if "$qadir" != "" {
    cap copy "`logfile'" "`logs_dir'/run_tests.log", replace
    if _rc==0 di as text "Canonical log copied to: `logs_dir'/run_tests.log"
    else di as error "Could not copy log to `logs_dir'/run_tests.log (rc=`=_rc')"
}
else {
...
if "$target_test" == "" & "$repo_root" != "" {
    local histfile "$qadir/logs/test_history.txt"
    ...
    * Validate: qa directory must contain run_tests.do (sanity check)
    ...
    if _rc != 0 {
        ...
        file write history "Log:      logs/test_results_v`version'_`datestr'.log" _n
