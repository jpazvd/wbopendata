...
program def __wbod_query_metadata, rclass
...
version 9.0
	...
	*---------------------------------------------------------------------------
	** pull metadata using __wbod_api_read.ado
	local indicator_code "`indicator1'"
	if ("`indicator_code'" == "") local indicator_code "`indicator'"
	__wbod_api_read, list query("https://api.worldbank.org/v2/indicators/`indicator_code'") ///
	...
	* Initialize URL collection
	...
	cap qui __wbod_website, text(`"`source'"')
	if (_rc == 0) {
		local source = r(text)
	...
	cap qui __wbod_website, text(`"`note'"')
	if (_rc == 0) {
		local note = r(text)
    ...
    *noi __wbod_website, text(`"`note'"')  short(link)
	noi di in smcl  "{hline}"
	...
	if ("`linewrap'" != "") {
		...
		* Wrap name
		if (`wrap_name' == 1) {
			...
			cap __wbod_metadata_linewrap, text(`"`name'"') maxlength(`ml') prefix(name)
			if (_rc == 0) {
		...
		* Wrap description
		if (`wrap_description' == 1) {
			...
			cap __wbod_metadata_linewrap, text(`"`source'"') maxlength(`ml') prefix(description)
			if (_rc == 0) {
		...
		* Wrap note
		if (`wrap_note' == 1) {
			...
			cap __wbod_metadata_linewrap, text(`"`note'"') maxlength(`ml') prefix(note)
			if (_rc == 0) {
		...
		* Wrap source
		if (`wrap_source' == 1) {
			...
			cap __wbod_metadata_linewrap, text(`"`source'"') maxlength(`ml') prefix(source)
			if (_rc == 0) {
		...
		* Wrap topics
		if (`wrap_topic' == 1) {
			...
			cap __wbod_metadata_linewrap, text(`"`topictext'"') maxlength(`ml') prefix(topic)
			if (_rc == 0) {
