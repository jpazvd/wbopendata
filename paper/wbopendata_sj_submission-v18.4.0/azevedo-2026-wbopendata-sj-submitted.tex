...
% Submission version for Stata Journal
% =====================================================================
...
\sjsetissue{00}{00}{Month}{2026}

\inserttype[st0001]{article}

\title[Data Provenance in the Age of AI]{wbopendata: Programmatic Access to World Bank Open Data}
...
This principle has antecedents in the Stata ecosystem. \citet{drukker2006freduse}
demonstrated how a purpose-built command (\texttt{freduse}) could transform the
retrieval of Federal Reserve economic data from a manual browser task into a
scriptable, reproducible operation embedded in analytical code. \texttt{wbopendata}
extends this logic to the broader landscape of World Bank development indicators,
applying the same core insight---that data acquisition should be code---to a data
ecosystem an order of magnitude larger.
...
An early and influential precedent is \texttt{freduse} \citep{drukker2006freduse},
which demonstrated that economic data retrieval could be embedded directly in
Stata syntax with per-variable metadata stored as variable characteristics
(\texttt{char}). The design choices visible in \texttt{freduse}---scripted
acquisition, structured metadata, and Stata-native output---anticipate many of
the patterns adopted by \texttt{wbopendata} and its successors.
...
access through \texttt{unicefData} \citep{unicefdata2026}, which provides a triangulated suite of R, Python,
...
The influence has also been bidirectional: the discovery architecture introduced
by \texttt{unicefData}---offline catalog browsing with YAML-backed metadata and
clickable SMCL navigation---was subsequently adopted by \texttt{wbopendata}
(v18.0), demonstrating the portability of these design patterns across data
ecosystems.
...
\hangpara
\texttt{\underbar{ind}icator(\ststring)} specifies one or more World Bank indicator codes. Multiple indicators can be requested by separating codes with semicolons:\newline
\mbox{}\quad\texttt{indicator(SP.POP.TOTL;NY.GDP.PCAP.CD)}.
...
\hangpara
\texttt{date(\ststring)} provides an alternative time filter for series that
support quarterly or monthly periodicity (e.g., \texttt{date(2020Q1:2021Q4)}).

\hangpara
\texttt{\underbar{s}ource(\ststring)} restricts the query to a specific World
Bank data source by numeric ID (e.g., \texttt{source(2)} for World Development
Indicators).
...
\hangpara
\texttt{\underbar{nochar}} suppresses the dataset and variable characteristics
(\texttt{char}) that are written by default as of v18.1. See
Section~\ref{sec:reliability} for details on the characteristic metadata layer.

\subsubsection*{Country attributes}

\hangpara
\texttt{\underbar{bas}ic} adds region, administrative region, income level, and lending type variables to the downloaded data. This is the default behavior in v17.7+.
...
\hangpara
\texttt{iso}, \texttt{regions}, \texttt{adminr}, \texttt{income},
\texttt{lending} add individual attribute groups without the full set.
...
\subsubsection*{Metadata management (v18.0+)}

\noindent\textit{Sync preview (dry run):}

\hangpara
\texttt{sync} previews metadata changes without applying them. Compares the
local cache version against the latest available release and displays a
summary. This is the safe default---no files are modified.

\hangpara
\texttt{sync detail} extends the preview with per-source and per-topic
indicator breakdowns.

\hangpara
\texttt{sync force} forces a fresh preview by re-querying the API rather
than relying on cached diagnostics. Still a dry run.

\noindent\textit{Sync apply:}

\hangpara
\texttt{sync replace} applies the metadata synchronization---downloads the
latest YAML metadata release from the GitHub repository and updates the
local cache.

\hangpara
\texttt{sync replace force} forces a re-download regardless of the local
cache version, bypassing staleness checks.

\noindent\textit{Cache management:}

\hangpara
\texttt{checkupdate} queries the remote repository for the latest release
version and reports whether an update is available, without downloading
or modifying any files.

\hangpara
\texttt{cacheinfo} displays the cache directory location, current metadata
version, and last synchronization timestamp.

\hangpara
\texttt{clearcache} removes the local metadata cache entirely, forcing a
full re-download on the next sync or discovery command.

\noindent\textit{Data response cache (v18.2+):}

\hangpara
\texttt{nocache} bypasses the local data response cache for a single query,
forcing a fresh download from the World Bank API regardless of whether a
cached response exists.

\hangpara
\texttt{cachedays(\num)} sets the time-to-live for cached data responses
in days. The default is~7. For example, \texttt{cachedays(30)} retains
cached API responses for 30~days before re-fetching.

\hangpara
\texttt{cleardatacache} removes all cached API response files from the
local data cache directory.

\hangpara
\texttt{resetdatacache} expires all cached data entries without deleting
the underlying files. The next query for any indicator will re-fetch fresh
data from the API, but the old cache files remain available for inspection.

\hangpara
\texttt{verbose} enables targeted diagnostic output for cache and metadata
operations, displaying cache hit/miss status, file paths, and timing
information.

\subsubsection*{Graph metadata (v17.6+)}
...
\subsection{Discovery commands (v18.0)}\label{sec:discovery}

Version~18.0 introduces a suite of discovery commands that allow users to explore the World Bank Open Data catalog without downloading data. The discovery architecture follows the model introduced by \texttt{unicefdata} \citep{unicefdata2026}, which pioneered offline catalog browsing with YAML-backed metadata and clickable SMCL navigation in Stata. The \texttt{wbopendata} implementation extends this pattern to the World Bank's 71 data sources and 29{,}000+ indicators.
...
\hangpara
\texttt{sources} lists all World Bank data sources (databases) with indicator counts and clickable navigation links.

\hangpara
\texttt{allsources} lists all data sources without the default display limit,
showing the complete set of 71 databases.

\hangpara
\texttt{alltopics} lists all 21 World Bank topic categories with indicator counts and clickable navigation links.

\hangpara
\texttt{search(\ststring)} searches indicators by keyword, wildcard pattern, or regular expression. Multiple keywords joined with \texttt{+} require all terms to match. The prefix \texttt{\~{}} activates full regex syntax. Results include clickable links to inspect or download each indicator.

\hangpara
\texttt{searchsource(\num)} filters search results to a specific source ID. Can be used alone to browse all indicators in a source.

\hangpara
\texttt{searchtopic(\num)} filters search results to a specific topic ID.

\hangpara
\texttt{searchfield(\ststring)} restricts search to a metadata field:
\texttt{code}, \texttt{name}, \texttt{description}, \texttt{source},
\texttt{topic}, \texttt{note}, or \texttt{all} (default).
Multiple fields can be separated by semicolons.

\hangpara
\texttt{exact} requires an exact indicator code match rather than partial matching.

\hangpara
\texttt{detail} displays search results in wrapped block format with full indicator names and topic labels.

\hangpara
\texttt{limit(\num)} sets the maximum number of results to display (default~20).

\hangpara
\texttt{info(\ststring)} displays detailed metadata for a specific indicator, including its full name, source database, topic classifications, description, methodology notes with clickable URLs, and download commands.

These commands support the paper's central thesis by making indicator \emph{selection} itself a scripted, auditable process. Rather than browsing web portals manually and noting indicator codes on paper, researchers can document their search and selection process as executable Stata commands that can be inspected, replicated, and version-controlled. Verbatim outputs illustrating each discovery command are reported in Appendix~\ref{app:diagnostics:sources}--\ref{app:diagnostics:alltopics}.
...
\noindent\textbf{Indicator codes and variable names.}
World Bank indicator codes like \texttt{SI.POV.DDAY} contain periods,
which Stata does not permit in variable names. The command automatically
converts indicator codes to Stata-safe variable names by replacing periods
with underscores and converting to lowercase: \texttt{SI.POV.DDAY} becomes
\texttt{si\_pov\_dday}.
Both forms are stored: \texttt{r(indicator\#)} preserves the original API code for documentation and re-querying, while \texttt{r(varname\#)} provides the Stata variable name for use in analysis commands.
...
\begin{table}[htbp]
\centering
\caption{Stored results: core command}
\label{tab:stored}
...
Discovery commands (Section~\ref{sec:discovery}) return a separate set of
results documented in Table~\ref{tab:stored-discovery}.

\begin{table}[htbp]
\centering
\caption{Stored results: discovery commands (v18.0)}
\label{tab:stored-discovery}
\begin{tabular}{llp{6.5cm}}
\hline
Result & Type & Description \\
\hline
\multicolumn{3}{l}{\textit{\texttt{sources}}} \\
\texttt{r(n\_sources)} & scalar & Total number of data sources \\
\texttt{r(n\_indicators)} & scalar & Total indicators across all sources \\
\texttt{r(source\_codes)} & local & Space-separated list of source IDs \\
\texttt{r(source\_names)} & local & Compound-quoted list of source names \\
\hline
\multicolumn{3}{l}{\textit{\texttt{alltopics}}} \\
\texttt{r(n\_topics)} & scalar & Total number of topics \\
\texttt{r(topic\_ids)} & local & Space-separated list of topic IDs \\
\texttt{r(topic\_names)} & local & Compound-quoted list of topic names \\
\hline
\multicolumn{3}{l}{\textit{\texttt{search(pattern)}}} \\
\texttt{r(n\_results)} & scalar & Total matches found \\
\texttt{r(n\_displayed)} & scalar & Matches shown after limit \\
\texttt{r(first\_code)} & local & First matching indicator code \\
\texttt{r(codes)} & local & Space-separated indicator codes \\
\texttt{r(keyword)} & local & Search keyword used \\
\hline
\multicolumn{3}{l}{\textit{\texttt{info(code)}}} \\
\texttt{r(indicator)} & local & Indicator code \\
\texttt{r(name)} & local & Indicator name \\
\texttt{r(source\_id)} & local & Source database ID \\
\texttt{r(description)} & local & Full indicator description \\
\texttt{r(note)} & local & Methodology note \\
\hline
\multicolumn{3}{l}{\textit{All discovery commands}} \\
\texttt{r(cmd)} & local & Reproducible command string \\
\hline
\end{tabular}
\end{table}

These returns enable fully automated workflows: a script can download data, extract \texttt{r(name1\_stack)} for the graph title, \texttt{r(sourcecite1)} for the source note, and \texttt{r(latest)} for a coverage subtitle---all without hardcoding any metadata. Discovery returns further support automation by enabling scripts to programmatically enumerate available indicators, filter by source or topic, and extract metadata for selected series.
...
\subsection{Indicator discovery as auditable selection}\label{sec:workflow-discovery}

A fourth canonical workflow, introduced in version~18.0, demonstrates how
discovery commands make indicator \emph{selection} itself a scripted and
auditable process. In traditional workflows, researchers browse web portals,
scan indicator lists visually, and record codes manually---a process that
leaves no executable trace. Discovery commands replace this with a
...
Each step in this sequence is an executable Stata command that can be
preserved in a do-file, version-controlled, and rerun by independent
researchers. The discovery workflow thus extends the principle of data
acquisition as code (Section~\ref{sec:design}) upstream: not only is the
data \emph{download} scripted, but the process of \emph{finding and selecting}
indicators is documented as well. Stored results from each discovery command
(Section~\ref{sec:discovery}) enable programmatic enumeration of available
indicators, filtering by source or topic, and extraction of metadata for
selected series---supporting fully automated pipelines from discovery through
analysis.
...
Version~18.0 refactors the implementation into a modular architecture of 34 ado
files, with sub-routines (prefixed \texttt{\_\_wbod\_}) handling YAML parsing,
search, sync, cache management, and discovery commands. As of version~18.4, all
internal helpers adopt the \texttt{\_\_wbod\_} namespace (double underscore
prefix) to comply with Stata Journal naming conventions and avoid potential
conflicts with official Stata routines. This modular design improves
maintainability and enables independent testing of components.
...
\subsubsection*{Characteristic metadata (v18.1)}

Beginning with version~18.1, \texttt{wbopendata} embeds provenance and
indicator metadata directly into the \texttt{.dta} file using Stata's variable
characteristics (\texttt{char}) mechanism. This approach follows the precedent
established by \texttt{freduse} \citep{drukker2006freduse}, which stores
per-variable metadata as characteristics attached to each retrieved series.

Two layers of characteristics are written by default. At the dataset level,
\texttt{\_dta} characteristics record the command version, timestamp, username,
and the exact syntax used to generate the dataset. At the variable level, each
indicator variable carries characteristics for its source database, description,
methodology notes, citation, and topic classifications.

This design produces self-documenting datasets: a \texttt{.dta} file carries
enough metadata to identify what was requested, when, and by whom, without
requiring access to the original do-file or log. Because characteristics
persist through \texttt{save}/\texttt{use} cycles, this provenance information
travels with the data even when shared across teams or archived.

The characteristic layer complements rather than replaces the existing metadata
channels. Do-file syntax documents the researcher's intent; \texttt{r()} return
values support ephemeral programmatic workflows; and \texttt{char} values
provide persistent, machine-readable metadata embedded in the output file.
Users who prefer not to attach characteristics can suppress them with the
\texttt{nochar} option.
...
To ensure reliability, \texttt{wbopendata} ships with a two-layer validation
framework comprising 163~tests that enforce distinct but complementary
invariants. The testing philosophy draws on the software certification
principles of \citet{gould2001certification}; the full framework is
documented in Appendix~\ref{app:qa}.

\textbf{Layer~1: Feature validation} (\texttt{qa/run\_tests.do}).  The
primary QA suite exercises the command's features end-to-end against the
live World Bank API, covering indicator retrieval, pagination, caching
behavior, option handling, and fixes for historical issues.  This layer
answers the question: \emph{does the code work?}

\textbf{Layer~2: Documentation validation}
(\texttt{tests/test\_help\_examples.do}).  A second suite validates that
every example documented in the help file executes without error, ensuring
that documentation accuracy tracks code changes.  This layer answers a
different question: \emph{does the documentation match the code?}

Both layers share a common test harness pattern (explicit execution,
return-code inspection, pass/fail attribution), produce timestamped logs,
and enforce a version guard that prevents stale tests from running against
newer code.  Together they ensure that neither features nor documentation
...
or API endpoint deprecations. Layer~1 comprises 92 integrated tests
distributed across 15 categories, covering environment validation, data download
modes, output formatting, country metadata, regression protection, linewrap
graph annotation, update commands, topic queries, language options, population
projections, cache management, metadata synchronization, discovery commands,
characteristic metadata verification, error condition validation, extreme input
cases, and deterministic offline testing.
Layer~2 adds 71~tests organized along the same functional categories;
ten of the fifteen categories are exercised by both layers, while five
(ENV, REG, ERR, EXT, DET) are tested only in~Layer~1, reflecting
infrastructure and error-path validation that falls outside help-file
documentation (Table~\ref{tab:test-coverage}).

\begin{table}[htbp]
\centering
\caption{Test suite composition (163 tests across two validation layers)}
...
CHAR       & Characteristics   & 6  & 2  & \texttt{char} metadata, persistence, \texttt{nochar} \\
Advanced   & Advanced features & 6  & 2  & Projection, nobasic, nometadata, date \\
...
\floatnote{\emph{Note:} L1~= feature validation (\texttt{qa/run\_tests.do});
L2~= documentation validation (\texttt{tests/test\_help\_examples.do}).
Categories marked ``--'' are tested only in~L1.
See Appendix~\ref{app:qa} for full test definitions.}
...
\subsection{Metadata management}\label{sec:metadata}
...
\subsection{YAML metadata architecture (v18.0)}\label{sec:yaml}

Version~18.0 introduces a structured metadata layer that supports the discovery
commands described in Section~\ref{sec:discovery}. The metadata catalog is stored
in YAML files within a local cache directory, enabling offline catalog browsing
without network access.

Two primary YAML files constitute the metadata catalog:

\hangpara
\texttt{\_wbopendata\_indicators.yaml} contains metadata for all indicators
accessible through the World Bank API---over 29{,}000 records spanning 71 data
sources. Each record includes the indicator code, name, source database,
topic classifications, description, and methodology notes. At approximately
17~MB and 100{,}000+ lines, this file is the largest component of the local
cache.

\hangpara
\texttt{\_wbopendata\_parameters.yaml} stores country metadata (296 countries
and aggregates), source definitions, and topic classifications. This file
supports the \texttt{sources} and \texttt{alltopics} discovery commands and
provides the lookup tables used when enriching downloaded data with country
attributes.

The cache directory also maintains version-tracking files that record the
schema version, last synchronization timestamp, sync method, and a cumulative
history of all sync operations. This audit trail allows users and automated
pipelines to verify that the metadata driving indicator selection corresponds
to a known, documented state.

\subsubsection*{Performance}

Parsing a 17~MB YAML file imposes a non-trivial startup cost. On first
invocation, discovery commands require approximately 21~seconds to parse the
indicators file and build an in-memory frame cache. Subsequent calls within the
same Stata session retrieve results from the cached frame in under 0.5~seconds
(Stata~16+). This frame-caching strategy ensures that interactive workflows
involving repeated searches, source browsing, or metadata inspection remain
responsive after the initial parse.

\subsection{Metadata synchronization}\label{sec:sync}

The \texttt{sync} family of commands (documented in Section~\ref{sec:command})
provides version-controlled metadata updates with an explicit safety gate:
the default \texttt{sync} is a dry run that previews changes, while
\texttt{sync replace} applies them.  Three auxiliary
commands---\texttt{checkupdate}, \texttt{cacheinfo}, and
\texttt{clearcache}---provide version queries and cache housekeeping.
For backward compatibility, the legacy aliases \texttt{syncforce},
\texttt{syncpreview}, and \texttt{syncdryrun} remain functional with
deprecation warnings, as does \texttt{metadataoffline}, which is superseded
by the YAML metadata architecture and the discovery commands described in
Section~\ref{sec:discovery}.

The sync architecture attempts three download pathways in order of preference:
a Python-based canonical pipeline (when Python is available), a pure-Stata
fallback, and direct download from the GitHub releases page. This layered
strategy maximizes compatibility across institutional computing environments
with varying software restrictions while ensuring that at least one pathway
succeeds. Verbatim outputs of sync diagnostics and cache management commands
are reported in Appendix~\ref{app:diagnostics:sync}--\ref{app:diagnostics:checkupdate}.

\subsection{Data response cache (v18.2+)}\label{sec:datacache}

Version~18.2 introduces a local data response cache that stores API responses
on disk, keyed by a hash of the request parameters (indicator codes, country
filters, year range, and source identifier). When a subsequent query matches
a cached entry whose age is within the configured time-to-live, the command
returns the stored response without contacting the World Bank API. This
eliminates redundant network traffic during iterative analysis and reduces
sensitivity to transient API outages.

The cache time-to-live defaults to seven days and can be adjusted per query
with the \texttt{cachedays(\num)} option. Three housekeeping commands provide
explicit cache control: \texttt{cleardatacache} removes all cached response
files; \texttt{resetdatacache} marks all entries as expired without deleting
files, so that the next query re-fetches fresh data while preserving the old
responses for inspection; and \texttt{nocache} bypasses the cache for a
single query. The \texttt{verbose} option displays cache hit/miss status
and file paths during execution.

Beginning with version~18.3, discovery commands (\texttt{search},
\texttt{info}, \texttt{sources}, \texttt{alltopics}) resolve metadata from
cached YAML files when the metadata cache is current, avoiding API calls
entirely for catalog browsing operations. This cache-aware discovery layer
further reduces network dependency during interactive workflows.
...
71 databases, encapsulating API communication, pagination, caching, and metadata
...
an aspirational norm to a default behavior. Versions~18.0 through~18.3 extend this approach
with offline discovery commands for catalog browsing and indicator search,
a YAML-backed metadata architecture, version-controlled metadata synchronization,
publication-ready metadata handling, self-documenting datasets through variable
characteristics, a local data response cache with configurable time-to-live,
deterministic offline testing, and a two-layer validation framework
with 163~tests across feature and documentation validation.
...
This subsection shows examples of user-written commands
that rely on \texttt{wbopendata} internally.
...
\caption{GDP per capita (constant 2015 US\$) in Africa, 2009. Generated using \texttt{worldstat} \citep{clarke2012worldstat}, which calls \texttt{wbopendata} internally.}
...
\subsection{Metadata sync preview output}
\begin{stlog}
\input{sjlogs/ex_sync_preview.log.tex}\nullskip
\end{stlog}

\subsection{Discovery: Available sources}\label{app:diagnostics:sources}
\begin{stlog}
\input{sjlogs/ex_discovery_sources.log.tex}\nullskip
\end{stlog}

\subsection{Discovery: Indicator search}\label{app:diagnostics:search}
\begin{stlog}
\input{sjlogs/ex_discovery_search.log.tex}\nullskip
\end{stlog}

\subsection{Discovery: Indicator metadata}\label{app:diagnostics:info}
\begin{stlog}
\input{sjlogs/ex_discovery_info.log.tex}\nullskip
\end{stlog}

\subsection{Discovery: Topic categories}\label{app:diagnostics:alltopics}
\begin{stlog}
\input{sjlogs/ex_discovery_alltopics.log.tex}\nullskip
\end{stlog}

\subsection{Sync: Detailed metadata breakdown}\label{app:diagnostics:sync}
\begin{stlog}
\input{sjlogs/ex_sync_detail.log.tex}\nullskip
\end{stlog}

\subsection{Cache: Update check}\label{app:diagnostics:checkupdate}
\begin{stlog}
\input{sjlogs/ex_checkupdate.log.tex}\nullskip
\end{stlog}
...
to maintain the reliability of \texttt{wbopendata} over time. The framework
builds on principles articulated by \citet{gould2001certification}, who
established that statistical software certification requires systematic testing
where ``tests are never thrown away'' and reliability is demonstrated through
cumulative evidence rather than single-point verification. These practices
...
\subsection{Dual testing paradigm: live and deterministic}

The test suite employs two complementary testing paradigms. The live
integration tests described above detect upstream regressions in real time but
are inherently non-deterministic: results depend on current API responses,
network conditions, and the evolving state of upstream data.  To complement
this approach, version~18.1 introduces \emph{deterministic offline tests}
(category~DET) that execute against pre-recorded CSV fixtures stored in
\texttt{qa/fixtures/}, eliminating network dependency entirely.

Each fixture captures a specific API response state---for example, a known
dataset for a particular indicator-country-year combination, or an empty
response simulating a deprecated indicator.  The \texttt{offline()} option
directs the command to load data from the fixture directory rather than
querying the live API.  Tests using fixtures produce identical results
regardless of network conditions or upstream data revisions, enabling exact
value pinning and regression baselines that would be impossible with live
tests.

For error condition tests (category~ERR), the suite employs Stata's
\texttt{rcof} certification command, an undocumented testing primitive
consistent with the methodology described by
\citet{gould2001certification}.  Unlike \texttt{capture noisily}, which
merely records whether a command succeeded or failed, \texttt{rcof}
verifies the \emph{exact} return code produced:
...
\noindent This confirms that omitting a required option produces a syntax error
(rc~198) rather than a network timeout or other incidental failure.  The
distinction matters: the old \texttt{capture noisily} pattern would pass
even if the command errored for the wrong reason, while \texttt{rcof}
guarantees that the error originates from the intended validation logic.

Because \texttt{rcof} halts execution on mismatch, integration with the
test harness's ``continue on failure'' convention requires wrapping it in
\texttt{capture noisily}:
...
\noindent This pattern preserves \texttt{rcof}'s precision while allowing the
test suite to continue executing subsequent tests after a failure.

The combination of live and deterministic tests creates a dual-paradigm
framework: live tests surface upstream changes that require attention, while
deterministic tests provide stable, reproducible baselines.  Together with the
extreme-case tests (category~EXT), which exercise boundary conditions such as
minimal queries, large result sets, and edge inputs, these additions bring the
total suite to 92~tests across 15~categories.

\subsection{Test harness structure}

The primary QA suite is implemented in \texttt{qa/run\_tests.do}, which
supports full test runs, single-test execution, and verbose (trace) modes.
Tests are organized into functional categories that
...
\subsubsection{Test category definitions}

Table~\ref{tab:test-coverage} organizes the 163~tests into 15~functional
categories, ordered by a typical analytical workflow: environment setup,
data discovery, retrieval, post-processing, infrastructure, and validation.
Each category is defined below.

\begin{itemize}
  \item \textbf{ENV} --- \emph{Environment.}  Validates prerequisites: network
    connectivity, API reachability, and presence of local YAML metadata files.
  \item \textbf{DISC} --- \emph{Discovery.}  Exercises the indicator search and
    metadata lookup subsystem: \texttt{sources}, \texttt{alltopics},
    \texttt{search()}, \texttt{info()}, and their filtering options
    (\texttt{searchsource}, \texttt{searchtopic}, \texttt{searchfield},
    \texttt{exact}, \texttt{detail}).
  \item \textbf{DL} --- \emph{Download modes.}  Tests core data retrieval paths:
    single-indicator, multi-indicator, country-specific, and topic-based
    downloads in both wide and long layouts.
  \item \textbf{FMT} --- \emph{Output format.}  Verifies output structure options
    including \texttt{long}, \texttt{year()}, and \texttt{latest}.
  \item \textbf{CTRY} --- \emph{Country options.}  Tests metadata enrichment via
    \texttt{match()}, \texttt{full}, \texttt{geo}, \texttt{capital},
    ISO code handling, and regional/income classification variables.
  \item \textbf{TOPIC/LANG} --- \emph{Topics \& language.}  Covers topic-filtered
    downloads and non-English metadata retrieval (\texttt{language(es)}).
  \item \textbf{LW} --- \emph{Linewrap.}  Validates the \texttt{linewrap()} graph
    annotation system: metadata field wrapping, \texttt{maxlength()},
    multi-indicator stacking, and \texttt{latest} scalar returns.
  \item \textbf{CHAR} --- \emph{Characteristics.}  Verifies that Stata
    \texttt{char} metadata is attached to datasets and variables by default,
    persists across operations, and is suppressed by \texttt{nochar}.
  \item \textbf{Advanced} --- \emph{Advanced features.}  Tests specialized
    options: \texttt{projection}, \texttt{nobasic}, \texttt{nometadata},
    \texttt{describe}, and date handling.
  \item \textbf{CACHE/SYNC} --- \emph{Cache \& sync.}  Exercises the full cache
    lifecycle (\texttt{nocache}, \texttt{cachedays()}, \texttt{cleardatacache},
    \texttt{resetdatacache}), metadata synchronization (\texttt{sync},
    \texttt{checkupdate}), and cross-validation between cache layers.
  \item \textbf{UPD} --- \emph{Maintenance.}  Tests administrative commands
    including legacy \texttt{update} aliases, \texttt{describe}, and sync
    interoperability.
  \item \textbf{REG} --- \emph{Regression.}  Permanent tests for historically
    resolved bugs (issues \#33, \#45, \#46, \#51), ensuring fixes are never lost.
  \item \textbf{ERR} --- \emph{Error conditions.}  Validates that invalid inputs
    produce correct return codes using Stata's \texttt{rcof} certification command.
  \item \textbf{EXT} --- \emph{Extreme cases.}  Exercises boundary conditions:
    minimal queries, large result sets, and unusual input combinations.
  \item \textbf{DET} --- \emph{Deterministic.}  Offline tests using pre-recorded
    CSV fixtures, enabling exact value pinning independent of network state.
\end{itemize}
...
bugs as permanent regression tests, following \citeauthor{gould2001certification}'s
principle that ``tests are never thrown away''.
Issues such as failures in multi-indicator queries, metadata parsing errors,
incorrect handling of deprecated indicators, or option incompatibilities are
encoded as named test cases (e.g., \texttt{REG-33}, \texttt{REG-45},
\texttt{REG-46}, \texttt{REG-51}).
...
Tests covering maintenance commands (\texttt{sync}, \texttt{checkupdate},
\texttt{sync replace}, and the legacy \texttt{update} aliases) verify that
such changes are detected and propagated through the local cache. Deprecated
indicators are expected to trigger informative
...
continuous validation---consistent with \citeauthor{gould2001certification}'s
demonstration that rigorous software certification is achievable through
disciplined testing practices rather than formal automation. Automated test execution, regression protection, and
...
\subsection{Help-file example validation}

A complementary test suite, \texttt{tests/test\_help\_examples.do}, validates
that every example documented in the help file (\texttt{wbopendata.sthlp})
executes without error. This suite comprises 71~tests organized in 12~categories:

\begin{itemize}
  \item \textbf{Categories~1--3}: Discovery commands, deprecated options, and cache
    management (local YAML lookups, no API calls required).
  \item \textbf{Categories~4--6}: Live data downloads testing indicator, country,
    topic, and multi-indicator queries with various option combinations.
  \item \textbf{Categories~7--8}: Named click-to-run examples and the end-to-end
    discovery workflow (sources $\to$ search $\to$ info $\to$ download).
  \item \textbf{Categories~9--12}: Inline example replay with data assertions,
    stored results verification, characteristic metadata checks, and data
    response cache workflows.
\end{itemize}

The script includes a version guard that compares the test version against
the resolved \texttt{wbopendata.ado} version and exits on mismatch,
preventing stale tests from running against newer code. Two execution modes
are supported: \emph{dev} mode (default) manipulates the \texttt{adopath}
to prioritize development source files; \emph{installed} mode tests the
\texttt{net install}-ed copy from PLUS. This dual-mode design ensures that
both development and release configurations are validated.

The help-file suite serves a distinct purpose from the QA suite: while
\texttt{run\_tests.do} validates \emph{features}, \texttt{test\_help\_examples.do}
validates \emph{documentation}. If a code change breaks a help-file example,
this suite detects the divergence before users encounter it.
