...
% =====================================================================
\begin{abstract}
In April 2010, the World Bank launched its Open Data Initiative, providing unrestricted access to development statistics via a public API. Within a year, \texttt{wbopendata} was released to translate this API into a Stata-native interface. Fifteen years later, the command provides access to over 29{,}000 indicators from 51 databases spanning 296 countries and regions with time series from 1960 to present. \texttt{wbopendata} supports five download modes, three languages (English, Spanish, French), flexible output formats (wide/long), transparent caching, and publication-ready metadata formatting. Version 17.7.1 adds graph metadata line-wrapping and default country-context attributes. This article documents the command's syntax, provides reproducible examples, and describes a live test suite (44 tests across 13 categories). The command exemplifies key principles for sustainable research infrastructure: stability through backward compatibility, usability through domain-specific design, and reproducibility through scripted data access.
\end{abstract}
...
\begin{description}
  \item[\texttt{indicator(string)}] One or more indicator codes from any of the 51 databases (29{,}000+ series), separated by semicolons. Example: \texttt{indicator(NY.GDP.MKTP.CD;SP.POP.TOTL)}.
...
\texttt{wbopendata} is an r-class command that stores results in \texttt{r()}. Following Stata's conventions for stored results (see \texttt{[P] return}), results are organized into macros (text strings) and scalars (numeric values). Naming follows Stata's recommended patterns: mathematical concepts use subscript notation (e.g., \texttt{\_ncountries}), while programming concepts use descriptive English names (e.g., \texttt{varname}, \texttt{indicator}).

\subsection{Macros: Indicator metadata}

For each requested indicator, where \texttt{\#} = 1, 2, \ldots{} indexes multiple indicators:

\begin{table}[htbp]
\centering
\caption{Stored macros for indicator metadata}
\label{tab:r-macros}
\begin{tabular}{lp{9cm}}
\hline
Macro & Description \\
\hline
\texttt{r(varname\#)} & Stata-safe variable name derived from indicator code (dots replaced with underscores) \\
\texttt{r(indicator\#)} & Original indicator code as requested \\
\texttt{r(varlabel\#)} & Indicator label (short name) from the API \\
\texttt{r(source\#)} & Source database identifier \\
\texttt{r(topics\#)} & Topic identifier, if specified in the request \\
\texttt{r(year\#)} & Year range requested \\
\texttt{r(time\#)} & Name of the time variable in the returned dataset \\
\texttt{r(name)} & Combined indicator codes (semicolon-delimited, normalized to lowercase with underscores) \\
\hline
\end{tabular}
\end{table}

These macros enable programmatic access to metadata for constructing graph titles, table headers, and documentation. For example, after requesting GDP data, \texttt{`r(varlabel1)'} contains the full indicator name suitable for a figure title.

\subsection{Scalars and macros: With \texttt{latest} option}

When \texttt{latest} is specified, additional results summarize the temporal coverage of the retained observations:

\begin{table}[htbp]
\centering
\caption{Stored results with \texttt{latest} option}
\label{tab:r-latest}
\begin{tabular}{llp{7cm}}
\hline
Result & Type & Description \\
\hline
\texttt{r(latest)} & macro & Formatted subtitle string, e.g., ``Latest Available Year, 186 countries (avg year 2019.6)'' \\
\texttt{r(latest\_ncountries)} & macro & Number of countries with non-missing data \\
\texttt{r(latest\_avgyear)} & macro & Average year across retained observations (formatted to one decimal) \\
\texttt{r(latest\_year)} & macro & Maximum year among retained observations \\
\hline
\end{tabular}
\end{table}

These results support automated figure annotation. A typical workflow extracts \texttt{r(latest)} directly into a graph subtitle, ensuring that data currency is documented without manual editing:

\begin{stlog}
\begin{verbatim}
. wbopendata, indicator(SI.POV.DDAY) clear long latest
. local subtitle "`r(latest)'"
. twoway ..., subtitle("`subtitle'")
\end{verbatim}
\end{stlog}

\subsection{Macros: With \texttt{linewrap} option}

The \texttt{linewrap()} option generates publication-ready text for graph titles and notes. For each indicator (\texttt{\#} = 1, 2, \ldots) and each requested field:

\begin{table}[htbp]
\centering
\caption{Stored macros with \texttt{linewrap} option (default \texttt{stack} format)}
\label{tab:r-linewrap}
\begin{tabular}{lp{8.5cm}}
\hline
Macro & Description \\
\hline
\texttt{r(name\#\_stack)} & Indicator name, wrapped for use in \texttt{title()} \\
\texttt{r(description\#\_stack)} & Full indicator definition, wrapped for captions \\
\texttt{r(note\#\_stack)} & Source notes from the API \\
\texttt{r(source\#\_stack)} & Source organization information \\
\texttt{r(topic\#\_stack)} & Topic classification \\
\texttt{r(sourcecite\#)} & Clean organization name for attribution, e.g., ``World Bank (WB)'' \\
\hline
\end{tabular}
\end{table}

The \texttt{\_stack} suffix indicates formatting compatible with Stata's \texttt{title()} option, where multiple quoted strings stack vertically: \texttt{"line1" "line2" "line3"}. With \texttt{linewrapformat(all)}, additional formats are returned:

\begin{description}
  \item[\texttt{r(field\#\_newline)}] Text with embedded newline characters (\texttt{\char`\\n}).
  \item[\texttt{r(field\#\_nlines)}] Count of lines after wrapping (useful for dynamic layout).
  \item[\texttt{r(field\#\_line1)}, \texttt{r(field\#\_line2)}, \ldots] Individual lines for custom formatting.
\end{description}

\subsection{Design rationale}

The stored results architecture reflects several design principles:

\begin{enumerate}
  \item \textbf{Composability}: Results can be directly substituted into subsequent commands without intermediate processing. Graph metadata flows seamlessly from API to figure.
  \item \textbf{Transparency}: Coverage statistics (\texttt{r(latest\_ncountries)}, \texttt{r(latest\_avgyear)}) make data limitations explicit rather than hidden.
  \item \textbf{Indexing for multiple indicators}: The \texttt{\#} suffix follows Stata's convention for commands that return variable-length results, enabling loops over multiple indicators.
  \item \textbf{Separation of raw and formatted}: Raw values (e.g., \texttt{r(indicator1)}) coexist with presentation-ready versions (e.g., \texttt{r(name1\_stack)}), supporting both programmatic and publication workflows.
\end{enumerate}
...
\subsection{Data with metadata}

Metadata is the backbone of reproducible research and the greatest strength of official statistics. It is what transforms a dataset from a mere collection of numbers into an interpretable, citable, and auditable resource—and what distinguishes authoritative institutional data from informal compilations. Metadata is what elevates data to become an authoritative source of truth in a particular domain: without it, users cannot verify units, temporal coverage, methodological revisions, or data provenance; with it, they can trace every figure back to its source and understand precisely what was measured, how, and when. The World Bank's data infrastructure follows the Statistical Data and Metadata Exchange (SDMX) standard—an ISO standard (ISO 17369:2013) endorsed by seven international organizations including the IMF, Eurostat, OECD, and the United Nations. SDMX provides a common vocabulary for describing statistical concepts, dimensions, and attributes, ensuring that indicators from different sources share consistent definitions.

When \texttt{wbopendata} retrieves an indicator, it simultaneously queries the metadata API to capture the indicator's name, definition, source database, and topic classification. This design choice reflects a core principle: data and metadata should travel together. Researchers who receive only the numeric values risk misinterpreting units, coverage, or revision status. By surfacing metadata automatically, \texttt{wbopendata} embeds documentation directly in the analytical workflow, reducing errors that arise when users manually transcribe definitions from web portals.

The following example introduces a minimal workflow: pull one GDP indicator from WDI for all economies and surface the built-in metadata (name, description, source, topic). By default, \texttt{wbopendata} returns data in wide format—one row per country, with year-specific values spread across columns (e.g., \texttt{yr1960}, \texttt{yr1961}, ...). This layout is convenient for cross-sectional snapshots but can be reshaped to long format using the \texttt{long} option when panel analysis is required. This example gives new users a reproducible starting point that shows what the command returns with no additional options.

\begin{stlog}
\input{sjlogs/ex_single_indicator.log.tex}\nullskip
\end{stlog}

This run confirms the default wide layout and the built-in metadata block (name, description, source, topic) that appear without extra options. The metadata returned here—indicator name, detailed definition, source database, and topic—are precisely the SDMX concept attributes that enable downstream users to verify what they are analyzing. This is especially valuable in institutional settings where audit trails require explicit documentation of data provenance. Rather than appending footnotes manually, researchers can programmatically extract \texttt{r(name1)}, \texttt{r(source1)}, and \texttt{r(description1)} for tables, figures, and replication archives.
...
This example keeps only the most recent non-missing observation per country. The algorithm works as follows: for each country, observations with missing values are dropped, then rows are sorted by country and year, and only the last observation (the most recent year with valid data) is retained. Because data availability varies across countries, the resulting dataset contains observations from different years—some countries may report 2023 data while others have 2019 as their latest. This heterogeneity is captured in the stored results: \texttt{r(latest\_avgyear)} returns the average year across all retained observations, providing a summary measure of data currency.

When multiple indicators are requested, a key design choice distinguishes \texttt{wbopendata} from some other data access tools: the algorithm keeps only observations where \textit{all} indicators have non-missing values, ensuring that each country's observation comes from a \textbf{common year across all requested indicators}. This means that if a user requests poverty and GDP data, each country will have both values from the same year—the most recent year where both indicators are available. This design prioritizes comparability over recency, avoiding the pitfall of comparing a 2023 poverty rate with a 2018 GDP figure for the same country. The stored scalars \texttt{r(latest)}, \texttt{r(latest\_ncountries)}, and \texttt{r(latest\_avgyear)} enable automated construction of figure subtitles and data-quality notes that transparently document coverage.

\begin{stlog}
\input{sjlogs/ex_latest_option.log.tex}\nullskip
\end{stlog}

The \texttt{latest} option prunes to most-recent values per country (not a single common year across all countries, but a common year across indicators within each country) and exposes coverage scalars that can feed graph subtitles or data-quality notes. Future versions may introduce additional filtering options such as \texttt{circa} (find closest available year to a target) or \texttt{mrv(\#)} (keep N most recent values per country), following patterns established in related tools like \texttt{unicefData}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figs/wbopendata_example01.pdf}
\caption{Mobile cellular subscriptions per 100 people (latest available year). Created using \texttt{wbopendata, indicator(IT.CEL.SETS.P2) latest clear} merged with geographic shape data and visualized using the \texttt{spmap} command. Generated by \texttt{wbopendata\_examples example01} from the help file.}
\label{fig:map}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figs/wbopendata_example04.pdf}
\caption{Poverty headcount ratio (\$1.90/day) vs.\ GDP per capita (PPP), with regional aggregates labeled. Data retrieved using \texttt{wbopendata, indicator(SI.POV.DDAY;NY.GDP.PCAP.PP.KD) latest} and exported to PDF using the \texttt{twoway scatter} and \texttt{graph export} commands. Generated by \texttt{wbopendata\_examples example04} from the help file.}
\label{fig:scatter}
\end{figure}
...
The \texttt{worldstat} module demonstrates the extensibility of \texttt{wbopendata}. While \texttt{worldstat} includes built-in statistics for commonly used indicators (GDP, maternal mortality, years of schooling), it seamlessly incorporates any of the 29{,}000+ World Bank indicators via \texttt{wbopendata}. Users can visualize both macroeconomic aggregates and microeconomic survey-based statistics with minimal setup.
