*******************************************************************************
*! __wbod_check_version v1.1.0  22Feb2026
*! Check remote metadata release tag vs local cache
*******************************************************************************

program define __wbod_check_version, rclass
    version 14.0
...
program define _wbopendata_parse_github_json, rclass
    ...
    *     "tag_name"         â† key line (contains "tag_name")
    *     "v17.7.1",         â† value line (starts with "v)
    tempname fh
    ...
    while r(eof) == 0 {
        if `found_key' {
            ...
            * â€” avoids all quoting issues with substr/inrange on
