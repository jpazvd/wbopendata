...
program def __wbod_query, rclass
...
quietly {
    ...
    *       to underscores in the key construction below.
    ...
    if ("`offline'" == "" & "`nocache'" == "") {
        ...
        * Check manifest for TTL â€” only file I/O is protected by capture
...
***************************************************
    ...
    if ("`l2'" == "") {		
		...
		cap : __wbod_api_read , query("https://api.worldbank.org/v2/Indicators/`indicator'") ///
			nopreserve ///
			list ///
