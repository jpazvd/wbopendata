...
program define __wbod_yaml_metadata, rclass
    version 14.0
    ...
    * Find the indicator (case-insensitive)
    ...
    if (r(N) == 0) {
        * Not found in YAML â€” caller should fall back to API
    ...
    * Parse topics (semicolon-separated in YAML â†’ topic1, topic2, topic3)
    local _topic1 ""
    local _topic2 ""
    local _topic3 ""
    if (`"`_topic'"' != "" & `"`_topic'"' != ".") {
    ...
    * Return values matching __wbod_query_metadata interface
