...
program define __wbod_query_indicators , rclass
...
quietly {
	...
	************************************************************************
	...
	forvalues p = `min'(1)`pages' {
		...
		local commandline `" __wbod_api_read ,  `linep' `linepp'  `linequery' list parameter( indicator?id name topic?id `sourcePar') `verbose' "'
...
*******************************************************************************
* v 15.2  	8Mar2019               by Joao Pedro Azevedo   
*	query to country attributes using API (requires __wbod_api_read.ado)
...
__wbod_query_indicators , indicator("2.0.cov.Math.pl_3.prv")
tab indicatorcode
list

__wbod_query_indicators , indicator("2.0.cov.Math.pl_3.prv") source
tab indicatorcode
list

__wbod_query_indicators , indicator(BX.GSR.TOTL.CD)
tab indicatorcode
list

__wbod_query_indicators , indicator(BX.GSR.TOTL.CD) source
tab indicatorcode
list
...
__wbod_query_indicators , per_page(10) page(1)
tab indicatorcode


__wbod_query_indicators , per_page(10) page(2)
tab indicatorcode


__wbod_query_indicators , per_page(10) page(100)
tab indicatorcode


__wbod_query_indicators , per_page(20) page(1)
tab indicatorcode
...
__wbod_query_indicators, per_page(20) page(2)
tab indicatorcode

__wbod_query_indicators, per_page(70) 
tab indicatorcode


__wbod_query_indicators, per_page(200) page(1)
tab indicatorcode

__wbod_query_indicators, per_page(2000) 
tab indicatorcode
...
__wbod_query_indicators, per_page(1) cmax(10)
tab indicatorcode

__wbod_query_indicators, per_page(1) page(647) cmax(653) noi nopreserve
tab indicatorcode

/* break */
set more off
__wbod_query_indicators, per_page(1) page(7687) cmax(16638) nopreserve using(tmp3tmp.csv)
tab indicatorcode

/* break */
__wbod_query_indicators, per_page(1) page(7680) cmax(7693) noi nopreserve using(tmp.csv)
tab indicatorcode


__wbod_query_indicators, per_page(1) page(1726) cmax(1728) noi nopreserve source
tab indicatorcode

*set trace on
__wbod_query_indicators, per_page(1) page(1728) cmax(1728) noi nopreserve verbose



__wbod_query_indicators, per_page(1) cmax(100) 
tab indicatorcode


clear
set more off
__wbod_query_indicators, per_page(1) cmax(16638) using(tmp2tmp)

set more off
__wbod_query_indicators, per_page(1) cmax(700)
...
__wbod_api_read, list query("http://api.worldbank.org/v2/indicators/BX.GSR.TOTL.CD") ///
...
__wbod_api_read, list query("http://api.worldbank.org/v2/indicators/6.1_LEG.CA") ///
...
__wbod_query_indicators, indicator("6.1_LEG.CA") nopreserve

__wbod_query_indicators, indicator("6.0.GDPpc_constant") nopreserve
...
__wbod_query_indicators, indicator(IN.HLTH.HIVDEATH.EST) nopreserve

__wbod_api_read, list query("http://api.worldbank.org/v2/indicators/IN.HLTH.HIVDEATH.EST") ///
		parameter( indicator?id name topic?id source?id sourceNote sourceOrganization ) ///
		verbose 
return list
