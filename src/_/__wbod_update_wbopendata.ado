...
program define __wbod_update_wbopendata, rclass
...
*    }
	...
	**********************************************
	...
	qui if ("`query'" == "") & ("`check'" != "") & ("`update'" != "") & (("`all'" == "") | ("`force'" == ""))  {
		...
		__wbod_api_read , query("https://api.worldbank.org/v2/countries/") ///
			nopreserve ///
			single ///
		...
		if ("`nodisplay'" == "") {
			...
			qui if ("`detail'" != "") {
				...
				noi __wbod_update_indicators, noindlist nosthlp1 nosthlp2 `query' `check'
		...
		* Write YAML parameters file
		...
		file write `out' `"# Auto-generated by __wbod_update_wbopendata.ado on `datef' `time'"' _n
		file write `out' "" _n
	...
	**********************************************
	...
	qui if ("`query'" == "") & ("`update'" != "") & (("`all'" != "") | ("`force'" != "")) {
		...
		if ((`number_indicators' == `newnumber') & (`ctrymetadata'==`newctrymetadata')) & ("`force'" == "") {
			noi __wbod_update_wbopendata, update check
		}
		...
		if ((`number_indicators' != `newnumber') | (`ctrymetadata'!=`newctrymetadata')) | ("`force'" != "") {
			...
			if ("`nodisplay'" == "")  {
				...
				* Refresh YAML first (Stata-only) when forcing full update
				if ("`force'" != "") {
					cap noi __wbod_refresh_yaml, replace
					if (_rc != 0) {
				...
				qui if ("`detail'" != "") | ("`force'" != "") {
					...
					* call __wbod_update_indicators.ado
					noi __wbod_update_indicators, `query' `check' `nohlp2'
					local newsourceid = r(sourceid)
					local newtopicid = r(topicid)
					local newsourcereturn = r(sourcereturn)
					local newtopicreturn  = r(topicreturn)
			...
			* Write YAML parameters file
			...
			noi __wbod_update_countrymetadata , `ctrylist'
...
*	change indicators update; function __wbod_update_indicators.ado replace  
