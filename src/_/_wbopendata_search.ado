*******************************************************************************
*! _wbopendata_search v2.5.0  04Feb2026
*! Search indicators from cached YAML with wildcards, filters, and SMCL nav
*******************************************************************************

program define _wbopendata_search, rclass
    version 14.0
    syntax [anything(name=keyword)] [, LIMIT(integer 20) SOURCE(string) ///
        TOPIC(string) FIELD(string) EXACT DETAIL]
        ...
        *-----------------------------------------------------------------------
        ...
        if ("`kw'" != "") {
            if ("`exact'" != "") {
                ...
                * Concatenate all searchable fields for easier matching
