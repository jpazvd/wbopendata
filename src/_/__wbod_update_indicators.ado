...
program define __wbod_update_indicators, rclass
...
if ("`file'" == "") {
	...
	__wbod_api_read_indicators, update preserveout file1(`file1')  file2(`file2') `check' `query'
...
if ("`nosthlp1'" == "") {
	...
	*----------------------------------------------
	cap noi __wbod_sources, limit(10000)
	if (_rc != 0) {
		local source_codes ""
		local source_names ""
		local n_sources = 0
	...
	*----------------------------------------------
	cap noi __wbod_topics, limit(10000)
	if (_rc != 0) {
		local topic_ids ""
		local topic_names ""
		local n_topics = 0
		...
		qui foreach topic1 in `levelsof2'  {	
			...
			file open `hlp`variable'`tc0''		using 	`help`variable'`tc0'' , write text replace
				...
				*** begin Indicator loop
					...
					*** Adjust websites
						cap: __wbod_website, text(`sourceorganization')
						if _rc == 0 {
							local sourceorganization = r(text)
						}

						cap: __wbod_website, text(`sourcenote')
						if _rc == 0 {
