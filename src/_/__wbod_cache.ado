...
program define __wbod_cache, rclass
    version 14.0
    ...
    if ("`update'" != "") {
        ...
        * 2. Optionally check for newer version on GitHub
        ...
        if (_rc == 0) {
            ...
            di as text "(Could not check GitHub for updates â€” offline or API unavailable.)"
        }
...
program define _wbopendata_cache_info, rclass
    ...
    * Frame cache status (Stata 16+)
    ...
    if (`c(stata_version)' >= 16) {
        ...
        if (_rc == 0) {
            ...
            di as text "  Frame cache:   " as text "(not loaded â€” will load on first search)"
        }
    }
    else {
    ...
    * Data cache status
    ...
    if (fileexists("`dc_dir'_manifest.txt")) {
        ...
        di as text "  Data cache:    " as text "(empty â€” queries will be cached on first download)"
    }
...
program define _wbopendata_reset_datacache
    ...
    if (!fileexists("`dc_dir'_manifest.txt")) {
        di as text "Data cache is empty â€” nothing to reset."
        exit 0
    }
