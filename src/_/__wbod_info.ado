...
*! v2.4.0: Parse Note field with __wbod_website to convert URLs to clickable links
...
program define __wbod_info, rclass
    version 16.0
    ...
    *---------------------------------------------------------------------------
    * Process Note and Description with __wbod_website to convert URLs to clickable links
    * Preserve original text for return list (without SMCL), use processed for display
    *---------------------------------------------------------------------------
    local note_plain `"`note'"'
    local desc_plain `"`desc'"'
    
    * Process note through __wbod_website (quietly to suppress its display)
    if (`"`note'"' != "N/A") {
        capture quietly __wbod_website, text(`"`note'"')
        if (_rc == 0 & `"`r(text)'"' != "") {
            local note `"`r(text)'"'
        }
    }
    
    * Process description through __wbod_website
    if (`"`desc'"' != "N/A") {
        capture quietly __wbod_website, text(`"`desc'"')
        if (_rc == 0 & `"`r(text)'"' != "") {
            local desc `"`r(text)'"'
        }
    }
    ...
    * Match __wbod_query_metadata return list for compatibility with describe
