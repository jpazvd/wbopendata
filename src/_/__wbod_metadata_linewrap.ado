...
program def __wbod_metadata_linewrap, rclass
...
version 14.0
	...
	* Options: stack | lines | newline | smcl | all
    ...
    quietly {
		...
		*-----------------------------------------------------------------------
		* Call __wbod_linewrap to split text into lines
		cap __wbod_linewrap, longstring(`"`text'"') maxlength(`maxlength')
		
		if (_rc != 0) {
			* __wbod_linewrap not available or error - return original as single line
			noi di as error "Warning: __wbod_linewrap not available, returning original text"
...
/********************************************************************************
USAGE:
	__wbod_metadata_linewrap, text("Long indicator name that needs wrapping") ///
...
EXAMPLES:
	...
	* Get indicator metadata and wrap the name for a graph
	__wbod_query_metadata, indicator(SH.DYN.MORT)
	__wbod_metadata_linewrap, text("`r(name)'") maxlength(45) prefix(title)
	twoway scatter y x, title(`r(title_stack)')
	
	* Wrap description with only stack format
	__wbod_metadata_linewrap, text("`r(description)'") maxlength(60) ///
