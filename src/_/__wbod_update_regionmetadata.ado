...
program define __wbod_update_regionmetadata , rclass
...
quietly {
	...
	************************************************************************
		...
		__wbod_api_read , query("http://api.worldbank.org/v2/region/") ///
			nopreserve ///
			single ///
		...
		*/
		...
		noi di in smcl in g ""
		noi di in smcl in g "{bf: Downloading region metadata...}"
		
		local maxp =  500
			
		************************************************************************

		file open `out' using `tmpregionlist' , text write replace
		...
		__wbod_api_read , query("https://api.worldbank.org/v2/region/") ///
			nopreserve ///
			single ///
			per_page(`maxp') ///
			parameter(page pages total)
		...
		forvalues p = 1(1)`pages' {
			...
			__wbod_api_read , query("https://api.worldbank.org/v2/region/") ///
				...
				noi di in smcl in g "{bf: Downloading region metadata... COMPLETED!}"
				noi di in smcl in g ""
				noi di in smcl in g "{bf: Processing region metadata...}"
		...
		tempfile tmpREGtmp_dta
		
		*******************************************************************************
		* create regionmetadata ado files
		*******************************************************************************
		
		insheet using `tmpregionlist' , delimiter("#") names clear 
		...
		*******************************************************************************
		...
		if ("`ctrylist'" == "ctrylist") {
			...
			noi di in smcl in g "{bf: Processing region list...}"
		...
		*******************************************************************************
		* create region sthlp files
		*******************************************************************************
		
		noi di in smcl in g "{bf: Processing region documentation...}"
		noi di in smcl in g ""
		...
		foreach variable in region adminregion incomelevel lendingtype {
			...
			__wbod_countrymetadata, match(countrycode) countryname full 
		...
		*******************************************************************************
		...
		noi di in smcl in g "{bf: Processing region documentation... COMPLETED!}"
