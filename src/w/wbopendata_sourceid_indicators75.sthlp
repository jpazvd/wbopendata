...
{synoptline}
{marker sourceid_AG.LND.FRLS.HA}
{synopt:{bf:{help wbopendata_sourceid##AG.LND.FRLS.HA:AG.LND.FRLS.HA} - Tree Cover Loss (hectares)}}

{synopt:{opt Source}}75 Environment, Social and Governance (ESG) Data{p_end}


{synopt:{opt Source Notes}}Shows year-by-year tree cover loss, defined as stand level replacement of vegetation greater than 5 meters, within the selected area. The tree cover loss data set is a collaboration of the University of Maryland, Google, USGS, and NASA, and us{p_end}

{synopt:{opt Source Organization}}Global Forest Watch. Tree Cover Loss (Hansen/UMD/Google/USGS/NASA).{p_end}
...
{synopt:{opt Source Notes}}A cooling degree day (CDD) is a measurement designed to track energy use. It is the number of degrees that a day's average temperature is above 18°C (65°F). Daily degree days are accumulated to obtain annual values.{p_end}
...
{synoptline}
{marker sourceid_EN.CLC.CSTP.ZS}
{synopt:{bf:{help wbopendata_sourceid##EN.CLC.CSTP.ZS:EN.CLC.CSTP.ZS} - Coastal protection}}
...
{synoptline}
{marker sourceid_EN.CLC.HDDY.XD}
{synopt:{bf:{help wbopendata_sourceid##EN.CLC.HDDY.XD:EN.CLC.HDDY.XD} - Heating Degree Days}}

{synopt:{opt Source}}75 Environment, Social and Governance (ESG) Data{p_end}


{synopt:{opt Source Notes}}A heating degree day (HDD) is a measurement designed to track energy use. It is the number of degrees that a day's average temperature is below 18°C (65°F). Daily degree days are accumulated to obtain annual values.{p_end}
...
{synopt:{bf:{help wbopendata_sourceid##EN.CLC.HEAT.XD:EN.CLC.HEAT.XD} - Heat Index 35}}

{synopt:{opt Source}}75 Environment, Social and Governance (ESG) Data{p_end}


{synopt:{opt Source Notes}}Total count of days per year where the daily mean Heat Index rose above 35°C. A Heat Index is a measure of how hot it feels once humidity is factored in with air temperature.{p_end}
...
{synopt:{bf:{help wbopendata_sourceid##EN.CLC.SPEI.XD:EN.CLC.SPEI.XD} - Standardised Precipitation-Evapotranspiration Index}}

{synopt:{opt Source}}75 Environment, Social and Governance (ESG) Data{p_end}


{synopt:{opt Source Notes}}The SPEI fulfills the requirements of a drought index since its multi-scalar character enables it to be used by different scientific disciplines to detect, monitor, and analyze droughts. Like the sc-PDSI and the SPI, the SPEI can measure droug{p_end}

{synopt:{opt Source Organization}}Global SPEI database (SPEIbase). {browse "https://spei.csic.es/database.html"}{p_end}


{synoptline}
{marker sourceid_EN.H2O.BDYS.ZS}
{synopt:{bf:{help wbopendata_sourceid##EN.H2O.BDYS.ZS:EN.H2O.BDYS.ZS} - Proportion of bodies of water with good ambient water quality}}
...
{synoptline}
{marker sourceid_EN.LND.LTMP.DC}
{synopt:{bf:{help wbopendata_sourceid##EN.LND.LTMP.DC:EN.LND.LTMP.DC} - Land Surface Temperature}}
...
{synoptline}
{marker sourceid_SD.ESR.PERF.XQ}
{synopt:{bf:{help wbopendata_sourceid##SD.ESR.PERF.XQ:SD.ESR.PERF.XQ} - Economic and Social Rights Performance Score}}
