...
class YAMLGenerator:
    ...
    def generate_indicators_yaml(self, indicators: List[Dict]) -> Path:
        ...
        # Compute checksum from the same serializer used for writing,
        # excluding the checksum field itself so the hash is reproducible.
        checksum = self._compute_checksum(yaml_data)
    ...
    def _setup_yaml_representer(self):
        """Register the custom string representer on SafeDumper."""
    ...
    def _serialize_yaml(self, data: Dict) -> str:
        """Serialize data to a YAML string using the same pipeline as _write_yaml."""
    ...
    def _compute_checksum(self, data: Dict) -> str:
        """Compute SHA-256 checksum of YAML content excluding the checksum field."""
        data_copy = copy.deepcopy(data)
        data_copy.get('_metadata', {}).pop('checksum_sha256', None)
        yaml_bytes = self._serialize_yaml(data_copy).encode('utf-8')
