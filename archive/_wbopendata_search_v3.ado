*******************************************************************************
*! _wbopendata_search_v3 v3.1.0  04Feb2026
*! Search indicators with frame-based session caching
*! Uses proven infix parsing, caches processed dataset in frame for speed
*! Falls back to per-call parsing for Stata < 16
*******************************************************************************

program define _wbopendata_search_v3, rclass
    version 14.0
    ...
    *---------------------------------------------------------------------------
    * Decide: Use frame cache (Stata 16+) or per-call parsing
    *---------------------------------------------------------------------------
    local use_frames = 0
    if (`c(stata_version)' >= 16 & "`nocache'" == "") {
        local use_frames = 1
    }
    ...
    if (`use_frames') {
        *-----------------------------------------------------------------------
        * FRAME CACHE approach (Stata 16+)
        * Cache the processed/collapsed indicator dataset in a frame
        * First call: parse YAML and save to frame
        * Subsequent calls: use cached frame directly
        *-----------------------------------------------------------------------
        ...
        * Check if frame already exists with valid data
        ...
        if (_rc == 0 & r(N) > 0) {
            * Frame exists and has data - check structure
        ...
        if (!`cache_loaded') {
            ...
            * Parse YAML using proven infix method
            _wbod_parse_yaml_ind "`yaml_path'"
        ...
        *-----------------------------------------------------------------------
        * FALLBACK: Parse YAML each call (Stata 14-15 or nocache)
        *-----------------------------------------------------------------------
        if ("`debug'" != "") {
    ...
    *---------------------------------------------------------------------------
    * Apply filters and search (common for both approaches)
    *---------------------------------------------------------------------------
    quietly {
    ...
    *---------------------------------------------------------------------------
    if (`n' == 0) {
        ...
        return local cache_method = cond(`use_frames', "frames", "nocache")
        exit 0
    }
