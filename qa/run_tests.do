...
foreach arg of local args {
    if upper("`arg'") == "VERBOSE" {
        ...
        * List all tests and exit
        ...
        di as text "  DISC-08  Search links use searchsource/searchtopic"
        di as text "  DISC-09  Search results include topic names"
        di as text "  DISC-10  Info multi-topic indicator return values"
        di as text ""
...
if $skip_test == 0 {
    cap noi {
        * Unfiltered baseline
        qui __wbod_search GDP
        local n_all = r(n_results)

        * Source filter (WDI = source 2)
        qui __wbod_search GDP, source(2)
...
* DISC-06: Indicator info lookup (comprehensive return value checks)
run_test "DISC-06" "Indicator info lookup"
if $skip_test == 0 {
    cap noi {
        qui __wbod_info, indicator(SP.POP.TOTL)
        ...
        * Topic fields (REG-09 regression: parent_stack fix)
        assert "`r(topic_ids)'" != ""
        assert `"`r(topics)'"' != ""
        assert `"`r(topics)'"' != "N/A"
        assert "`r(topic1)'" != ""

        * Description (should not be N/A for SP.POP.TOTL)
        assert `"`r(description)'"' != ""
        assert `"`r(description)'"' != "N/A"

        * Source fields
        assert "`r(source_name)'" != ""
        assert "`r(source_name)'" != "N/A"
        assert `"`r(source_org)'"' != ""
        assert `"`r(source_org)'"' != "N/A"

        * Note (should have content for SP.POP.TOTL)
        assert `"`r(note)'"' != ""

        di as text "Indicator:   `r(indicator)'"
        di as text "Name:        `r(name)'"
        di as text "Source ID:   `r(source_id)'"
        di as text "Source Name: `r(source_name)'"
        di as text "Source Org:  " `"`r(source_org)'"'
        di as text "Topic IDs:   `r(topic_ids)'"
        di as text "Topic 1:     `r(topic1)'"
        di as text "Topics:      " `"`r(topics)'"'
        di as text "Unit:        " `"`r(unit)'"'
        di as text "Limited:     `r(limited_data)'"
        di as text "Description: " substr(`"`r(description)'"', 1, 60) "..."
        di as text "Note:        " substr(`"`r(note)'"', 1, 60) "..."
    }
    if _rc == 0 test_pass
    else test_fail "Indicator info return values incomplete"
}
...
if $skip_test == 0 {
    cap noi {
        * wbopendata dispatches searchsource() → __wbod_search, source()
        ...
        * Internally dispatches to: __wbod_search , source(2)
        qui __wbod_search , source(2) limit(3)
        assert `r(n_results)' > 0
        assert "`r(source_filter)'" == "2"
        di as text "Browse source(2): `r(n_results)' results, first=`r(first_code)'"

        * Test searchtopic: wbopendata, search() searchtopic(3)
        * Internally dispatches to: __wbod_search , topic(3)
        qui __wbod_search , topic(3) limit(3)
        assert `r(n_results)' > 0
        ...
        * Both should work with empty keyword (browse mode)
        qui __wbod_search , source(57) limit(3)
        assert `r(n_results)' > 0
...
run_test "DISC-09" "Search results include topic names"
if $skip_test == 0 {
    cap noi {
        * Search WDI source (source 2) — most indicators have topics
        qui __wbod_search GDP, source(2) limit(10)
        local n_results = r(n_results)
        local topics_str = `"`r(topics)'"'

        assert `n_results' > 0

        * r(topics) should contain topic names, not all "-" or all empty
        * Count how many returned topics are non-empty
        local n_with_topic = 0
        local n_checked = min(`n_results', 10)
        forvalues i = 1/`n_checked' {
            local t : word `i' of `topics_str'
            if (`"`t'"' != "" & `"`t'"' != "-") {
                local n_with_topic = `n_with_topic' + 1
            }
        }

        di as text "Results: `n_results', checked: `n_checked', with topics: `n_with_topic'"
        di as text "Topics: " substr(`"`topics_str'"', 1, 80) "..."

        * At least some WDI indicators should have topic names
        assert `n_with_topic' > 0
    }
    if _rc == 0 test_pass
    else test_fail "Search results missing topic names (parent_stack bug)"
}
...
if $skip_test == 0 {
    cap noi {
        * NY.GDP.MKTP.CD typically has multiple topic IDs
        qui __wbod_info, indicator(NY.GDP.MKTP.CD)
